...
export default function Feed() {
  ...
  const [likedDropIds, setLikedDropIds] = useState(new Set())
  ...
  async function fetchLikedDrops(userId) {
    const { data } = await supabase.from('likes').select('drop_id').eq('user_id', userId)
    setLikedDropIds(new Set((data || []).map(l => l.drop_id)))
  }
  ...
  async function handleLike(dropId) {
    ...
    if (likedDropIds.has(dropId)) return
    setLikedDropIds(prev => new Set([...prev, dropId]))
    const { error } = await supabase.rpc('increment_like', { p_drop_id: dropId, p_user_id: user.id })
    if (error) {
      ...
      await supabase.from('drops')
        .update({ like_count: (drops.find(d => d.id === dropId)?.like_count || 0) + 1 })
        .eq('id', dropId)
    }
    ...
    <>
      ...
      {user && (
        <Link
          ...
          style={{
            color: notifCount > 0 ? '#00F5FF' : 'rgba(240,238,255,0.3)',
      ...
      {user
        ? (
          <span
            ...
            style={{ color: 'rgba(240,238,255,0.3)', fontSize: '11px', cursor: 'pointer' }}
          >
            sign out
          </span>
        ) : (
          <Link href="/login" style={{ color: 'rgba(240,238,255,0.3)', fontSize: '11px', textDecoration: 'none' }}>
            sign in
          </Link>
        )
      }
  ...
  function DropCard({ drop, isPromoted, isMerit }) {
    ...
    const isLiked = likedDropIds.has(drop.id)
    ...
    return (
      ...
      >
        ...
        {drop.thumbnail_url && (
          <div style={{ marginBottom: '16px' }}>
            <img
              ...
              alt={drop.title}
              style={{ width: '100%', height: '160px', objectFit: 'cover', border: '1px solid rgba(255,255,255,0.06)' }}
            />
          </div>
        )}

        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
          <Avatar url={drop.profiles?.avatar_url} username={drop.profiles?.username} size={28} />
        ...
        <Link href={`/drop/${drop.id}`} style={{ textDecoration: 'none' }}>
          <h2 style={{ color: '#F0EEFF', fontSize: '18px', fontFamily: 'monospace', fontWeight: 'normal', marginBottom: '10px', letterSpacing: '0.5px', lineHeight: '1.4' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginTop: '8px' }}>
          <button
            ...
            style={{
              ...
              color: isLiked ? '#FF2D78' : 'rgba(240,238,255,0.4)',
              fontSize: '11px', fontFamily: 'monospace', letterSpacing: '1px', padding: 0,
            }}
          >
            {isLiked ? '♥' : '♡'} {drop.like_count || 0}
          </button>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '16px', alignItems: 'center' }}>
            {visitUrl && (
              <a href={visitUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#FF2D78', fontSize: '11px', letterSpacing: '1px', textDecoration: 'none' }}>
                visit ↗
              </a>
            )}
            {user?.id === drop.user_id && (
              <>
                <Link href={`/drop/${drop.id}/edit`} style={{ color: 'rgba(0,245,255,0.5)', fontSize: '11px', letterSpacing: '1px', textDecoration: 'none' }}>
                  edit
                </Link>
                <button onClick={() => handleDelete(drop.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,45,120,0.5)', fontSize: '11px', fontFamily: 'monospace', letterSpacing: '1px', padding: 0 }}>
                  delete
                </button>
              </>
            )}
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#080810', fontFamily: 'monospace' }}>
      ...
      <nav style={{
        ...
        {isMobile && (
          <button
            ...
            style={{ background: 'none', border: 'none', color: '#00F5FF', fontSize: '20px', cursor: 'pointer', lineHeight: 1, padding: '0 4px' }}
          >
            {menuOpen ? '×' : '☰'}
          </button>
        )}
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {/* FILTER BAR */}
        <div style={{ display: 'flex', gap: '4px', marginBottom: '40px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <div style={{
                ...
                <div style={{ padding: '10px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <input
                    ...
                    style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', color: '#F0EEFF', fontFamily: 'monospace', fontSize: '12px', boxSizing: 'border-box' }}
                  />
                </div>
                {selectedTag && (
                  <div
                    onClick={() => { setSelectedTag(null); setTagDropdownOpen(false) }}
                    style={{ padding: '10px 16px', fontSize: '11px', color: '#FF2D78', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.06)', letterSpacing: '1px' }}
                  >
                    × clear filter
                  </div>
                )}
