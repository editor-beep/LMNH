...
export default function EditDrop({ params }) {
 ...
 useEffect(() => {
   supabase.auth.getUser().then(async ({ data }) => {
     ...
     if (!dropData) { window.location.href = '/feed'; return }

     if (dropData.user_id !== data.user.id) {
       window.location.href = `/drop/${params.id}`
       return
     }
     ...
     setForm({
       ...
       live_url: dropData.live_url || '',
       video_url: dropData.video_url || '',
       tools_used: dropData.tools_used || [],
       tags: (dropData.tags || []).join(', ')
     })
     setScreenshots(dropData.screenshot_urls || [])
     setLoading(false)
   })
 }, [])
   ...
   <main style={{
     ...
     <div style={{ maxWidth: '600px', margin: '0 auto' }}>
       ...
       <div style={{ marginBottom: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
         ...
         <Link href={`/drop/${params.id}`} style={{ color: 'rgba(240,238,255,0.4)', fontSize: '12px', textDecoration: 'none' }}>
           ← back
         </Link>
       </div>
